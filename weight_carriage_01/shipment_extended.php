<?
function shipment_extended($year1,$month1,$day1,$year2,$month2,$day2,$train,$vagon)
{
  ...
  $svagon=trim($vagon);
  echo "<br>";
  echo "<h3>Данные по отгрузке и взвешиванию тары</h3>";
  ...
  if($svagon!='')
	...
	$query = "SELECT w1.number,w1.brutto1,w1.weighing_mode,w1.speed,w1.status_weight,w1.sostav,w2.tare2,w2.is_manual_tare, ". 
          ...
          "DATE_FORMAT(w2.date_time,'%d-%m-%y'),DATE_FORMAT(w2.date_time,'%T'), ".
          "w2.sostav, w2.tare1, w2.weighing_mode, w2.speed ".
  ...
  $query_count="SELECT COUNT(*) ".substr($query,342);
  ...
  echo '<th style="background-color: #00945D; color: #ffffff; border: 1px solid #D3D3D3;" rowspan=2>№<th colspan=9>Отгрузка<th colspan=6>Тара - дополнительные данные';
  echo '<tr><th>Дата<th>Время<th>Состав<th>№ МПС<th>Тара,т<th>Брутто,т<th>Нетто,т<th>Режим<th>Скорость<th>Дата<th>Время<th>Состав<th>Трафарет,т<th>Режим<th>Скорость</tr>';
  echo '</thead>';
  echo '<tbody>';
  $i=1;
  $sum_netto=0;
  while($d=mysql_fetch_row($result))
  {
    ...
    if($d[7]==0)
    {
      $_speed = sprintf("%.1f",$d[15]);
      $_din_stat= ($d[14]==1 || $d[14]==3 || $d[14]==5) ? "стат":"дин";
    }
    else
    {
      $_speed="-";
    ...
    $_traf=sprintf("%.3f",$d[13]);
    echo"<tr><td>$i<td>$d[8]<td>$d[9]<td>$d[5]<td class=$str_is_manual>$d[0]<td>$tare<td>$brutto<td>$netto<td>$din_stat<td>$speed<td>$d[10]<td>$d[11]<td>$d[12]<td>$_traf<td>$_din_stat<td>$_speed";
    echo"</tr>";
    $i++;
  };
  echo "<tr>";
  echo "<td colspan=3><b>Всего отгружено</b><td colspan=4><td><b>$sum_netto</b><td colspan=8>&nbsp";
  echo "</tr>";
  echo"</tbody>";
  echo"</table>";
